...
def test_vcvs_stamp_basic():
    ...
    vcvs = VCVS(name="E1", a=1, b=2, c=3, d=0, gain=10.0)
...
def test_cccs_stamp_basic():
    ...
    cccs = CCCS(name="F1", a=1, b=2, c=3, d=0, gain=5.0)
...
def test_vccs_stamp_basic():
    ...
    vccs = VCCS(name="G1", a=1, b=2, c=3, d=0, gm=0.01)
...
def test_ccvs_stamp_basic():
    ...
    ccvs = CCVS(name="H1", a=1, b=2, c=3, d=0, rm=1000.0)
...
def test_controlled_sources_mna_flags():
    assert VCVS("E1", 1, 0, 2, 0, 5.0).is_mna is True
    assert CCCS("F1", 1, 0, 2, 0, 3.0).is_mna is True
    assert VCCS("G1", 1, 0, 2, 0, 0.01).is_mna is False
    assert CCVS("H1", 1, 0, 2, 0, 100.0).is_mna is True
