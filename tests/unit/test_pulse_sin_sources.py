...
def test_dc_source_constant_value():
    v = VoltageSource("V1", a=1, b=0, dc=10.0, source_type="DC")
...
def test_ac_source_sin_value():
    v = VoltageSource("V1", a=1, b=0, dc=0.0, amp=1.0, freq=1.0, 
...
def test_sin_basic_waveform():
    ...
    v = VoltageSource("V1", a=1, b=0, source_type="SIN", sin_params=sin_params)
...
def test_pulse_initial_value():
    ...
    v = VoltageSource("V1", a=1, b=0, source_type="PULSE", pulse_params=pulse_params)
...
def test_parser_reads_sin_netlist():
    ...
    assert sin_source.sin_params["amplitude"] == 5.0 #type: ignore
...
def test_parser_reads_pulse_netlist():
    ...
    assert pulse_source.pulse_params["v2"] == 5.0 #type: ignore
...
def test_old_dc_and_ac_sources():
    dc = VoltageSource("V1", a=1, b=0, dc=10.0, is_ac=False, source_type="DC")
    assert dc._value(0.0) == 10.0
    
    ac = VoltageSource("V1", a=1, b=0, dc=0.0, amp=1.0, freq=1.0, 
                      phase_deg=0.0, is_ac=True, source_type="AC")
